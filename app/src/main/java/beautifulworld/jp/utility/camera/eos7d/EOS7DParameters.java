...
public class EOS7DParameters implements Parameters
 {
  ...
  public int getNextWhiteBalance()
   {
    int next = WhiteBalance + 1;
    if (next >= WhiteBalanceValue.length) return WhiteBalanceValue[0];

    return WhiteBalanceValue[next];
   }
  
  public int getPrevWhiteBalance()
   {
    int next = WhiteBalance - 1;
    if (next > 0) return WhiteBalanceValue[WhiteBalanceValue.length - 1];
  ...
  public int getPrevAFMode()
   {
    int next = AFMode - 1;
    if (next < 0) return AFModeValue[AFModeValue.length - 1];
  ...
  public int getNextDriveMode()
   {
    int next = DriveMode + 1;
    if (next >= DriveModeValue.length) return DriveModeValue[0];

    return DriveModeValue[next];
   }

  public int getPrevDriveMode()
   {
    int next = DriveMode - 1;
    if (next < 0) return DriveModeValue[DriveModeValue.length - 1];
